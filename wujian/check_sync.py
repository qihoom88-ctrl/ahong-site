...
def main():
    ...
    # ── G-S4 已售出／已下架／暫停銷售不得留在列表上 ──────────
    #    第 104 行：status 不是「在售」就進 off，所以新增狀態值自動進射程。
    #    2026-09-06 新增「暫停銷售」（087 屋主辦理繼承）——狀態欄原本只有
    #    在售／已售出／已下架三格，沒有一格能表達「暫停但沒成交也沒撤案」，
    #    這正是 CAR-0902-02 的病根（資料結構逼人二選一），故先補格再填值。
    ghost = sorted(js_slugs & set(off))
    add("G-S4", "已售出／已下架／暫停銷售不在 data.js", not ghost,
